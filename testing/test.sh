...
cat $TMP_DIR/BBcalls.txt | grep -Ev "^[^,]*$|^([^,]*,){2,}[^,]*$"| sort  |tee $TMP_DIR/BBcalls2.txt > $TMP_DIR/BBcalls3.txt 
sort $TMP_DIR/BBcalls2.txt | uniq -c | awk '{print $2","$1}' >$TMP_DIR/BBcallcounts.txt && rm $TMP_DIR/BBcalls2.txt 
uniq $TMP_DIR/BBcalls3.txt > $TMP_DIR/BBcalls.txt && rm $TMP_DIR/BBcalls3.txt  
