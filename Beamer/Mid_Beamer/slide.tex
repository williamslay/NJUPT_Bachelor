        ...
        \begin{minipage}[t]{0.34\linewidth}
            {
                ...
                \preprocess\normalfont(\confs) $\rightarrow \confs$ 
                    \begin{itemize}  
            \scriptsize{\item  Instrumentation}
                            \\- \tiny{grey-box and white-box fuzzers}
                \scriptsize{\\-} \tiny{\alert{static}/dynamic}
            \scriptsize{\item Seed Selection}
                            \\- \tiny{weed out potentiallyredundant configurations}
            \scriptsize{ \item  Seed Trimming}
                            \\- \tiny{reduce the size of seeds}
            \scriptsize{\item  Preparing a Driver Application}
                            \\- \tiny{library Fuzzing, kernal Fuzzing}
                    \end{itemize} 
            }
        \end{minipage}
        \end{block}
