...
\begin{frame}{Motivation}
    \only<1>{
        \begin{itemize} 
            \item \structure {What Fuzzing is?}
                \\ \textbf{Definition:} Fuzzing is the execution of the PUT using input(s) sampled from an input space (the “fuzz input space”) 
                that protrudes the expected input space of the PUT\cite{manes2019art}.
                \\ - PUT: Program Under Test
            \item  \structure {classification of fuzzing}
                \\ - \textbf{Black-box Fuzzing} (no program analysis, no feedback)
                \\ - \textbf{White-box Fuzzing} (mostly program analysis)
                \\ - \textbf{Grey-box  Fuzzing} (no program analysis, but feedback)
        \end{itemize} 
    }
        ...
        \begin{picture}(320,250)
            ...
            \put(213,230){
                \begin{minipage}[t]{0.35\linewidth}
                {
                    \footnotesize{
                        \begin{itemize}  
                            \item - You have no view of the PUT
                            \item - But have some view of the input/output domain
                            \item - And the Fuzzing process is not changed according to some feedback  
                        \end{itemize} 
                    }
                }
                \end{minipage}
    ...
    \only<4>{
        ...
        \begin{figure}[htpb]
            \begin{center}
                \includegraphics[width=8.0cm,height=6.0cm]{pic/blackbox.pdf}
            \end{center}
        \end{figure} 
    }
\end{frame}
