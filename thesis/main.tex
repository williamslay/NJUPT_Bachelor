...
是利用了同一个LLVM的pass插件\cite{Pass}实现的，具体流程可参考图\ref{pic:pass}。为了延续图\ref{pic:AFLGo}中AFLGo的框架结构，
...
可达目标函数集的大小仅为350。考虑到设定随机数的取值范围为16k，这使得标识相撞的可能性较低。
...
本文仔细介绍了AFLGo的架构及其使用目标距离指标来实现定向目标覆盖的方法。然而，本文认为AFLGo的设计仍有改进的空间。
特别地，本文选择重新设计AFLGo的距离计算方式，以更全面地考虑函数内部不同基本块针对的调用概率，从而提高函数间距离的区分性。
此外，本文还采用增加能量调度影响指标，从而更公平地分配种子的能量。

本文以AFLGo的基本架构为出发点，关注AFLGo在机制设计中存在的偏见，并补充设计了旨在消除这些偏见的指标和导向策略。通过修改和集成
这些指标到AFLGo的架构中，本文实现了一个基于AFLGo架构的定向灰盒模糊测试工具。本文引入了更精细的函数间距离定义以及可达目标函数
集覆盖率指标，从而修正了AFLGo设计机制中存在偏见的判断原则。结合LLVM的Pass插件和Clang分析器，本文对被测试程序进行了更精细的程序分析，
并集成了新的指标计算部分。通过设计和整合更多的指标和策略，该工具可以在定向灰盒模糊测试中帮助用户更好地生成测试用例。

在实验中将实现的定向灰盒模糊测试工具用于系统的定向覆盖和漏洞检测，实验表明，设计实现的工具很好地完成了针对目标区域的
定向覆盖和测试任务，达到了研究设计目标。
\section{展望}
...
\begin{enumerate}[label=(\arabic*)]
	...
	基于其基本思路重新实现一个定向灰盒模糊测试工具都是一个很重要的研究方向，有助于进一步加强DGF的效能。
